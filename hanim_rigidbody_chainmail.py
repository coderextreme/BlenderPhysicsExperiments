...
def animate_humanoid(arm_obj):
    ...
    # Frames 1-60: T-Pose (Lets chainmail drop onto body)
...
def export_x3d_physics(filepath, arm_obj, skin_obj):
    ...
    xml = ['<?xml version="1.0" encoding="UTF-8"?>',
           ...
           # --- Map Blender Z-Up to X3D Y-Up natively ---
    ...
    def build_joint_xml(bone, is_root=False):
        ...
        # Inject Strict HAnimSegment for Volume/Collision
    ...
    # Valid ContainerField implementation for Skin
    ...
    # =========================================================
    # PROCEDURAL CHAINMAIL PHYSICS (RIGIDBODY GRID)
    # =========================================================
    GRID_SIZE = 12
    SPAN = 0.8
    ...
    # EXPLICIT GRAVITY: Because this is inside Z_UP_TO_Y_UP, gravity along local -Z correctly drops objects to global -Y.
    xml.append('<RigidBodyCollection gravity="0 0 -9.81">')
    
    # Kinematic Collision proxies (Mass=0 means they push objects but don't fall from gravity)
    ...
    # Chainmail Nodes (Mass=0.05 + useGlobalGravity explicitly ensures they fall around the body)
    for (r, c), (x, y) in nodes.items():
        node_def = f"ClothNode_{r}_{c}"
        rad = SPACING * 0.48
        xml.append(f'<RigidBody DEF="{node_def}" containerField="bodies" position="{x:.4f} {y:.4f} {Z_HEIGHT}" mass="0.05" useGlobalGravity="true" linearDampingFactor="0.6" angularDampingFactor="0.8">')
    ...
    xml.append('</Transform>') # Close Coordinate Converter
...
def main():
    ...
    export_path = os.path.join(home_dir, "hanim_rigidbody_chainmail.x3d")
