...
def generate_loa4_bones():
    """Generates standard HAnim LOA4 skeleton."""
    bones = []
...
def export_x3d_physics(filepath, arm_obj, skin_obj):
    ...
    xml = ['<?xml version="1.0" encoding="UTF-8"?>',
           ...
           '<head><component name="RigidBodyPhysics" level="2"/></head>',
           '<Scene>',
    ...
    # Kinematic Collision proxies bound to specific bones
    # Format: Bone -> (Radius, Offset_X, Offset_Y, Offset_Z)
    kinematic_colliders = {
        "hanim_skull": (0.11, 0, 0, 0.05),
        "hanim_vt4": (0.15, 0, 0, -0.05),      # Chest
        "hanim_vl3": (0.13, 0, 0, 0),          # Waist
        "hanim_pelvis": (0.17, 0, 0, 0),       # Hips
        "hanim_l_clavicle": (0.09, 0, 0.06, 0),# Left Shoulder
        "hanim_r_clavicle": (0.09, 0, -0.06, 0)# Right Shoulder
    }
    ...
    # Build HAnim Skeleton with Embedded Colliders
    def build_joint_xml(bone, is_root=False):
        ...
        # Inject Kinematic Proxy Geometry inside the Joint
    ...
    # 1. Register Kinematic Body Parts (Pushed by HAnim Animation)
    for bname in kinematic_colliders.keys():
        xml.append(f'<RigidBody DEF="RB_{bname}" mass="0">') # mass=0 makes it kinematic
        xml.append(f'<CollidableShape USE="CS_{bname}" containerField="geometry"/>')
        xml.append('</RigidBody>')
        
    # 2. Add Floor Collider
    xml.append('<RigidBody DEF="RB_Floor" mass="0" position="0 0 -0.05">')
    xml.append('<CollidableShape containerField="geometry"><Shape containerField="shape"><Box size="10 10 0.1"/></Shape></CollidableShape>')
    xml.append('</RigidBody>')

    # 3. Generate Chainmail Poncho Grid
    GRID_SIZE = 14
    SPAN = 1.0
    SPACING = SPAN / (GRID_SIZE - 1)
    Z_HEIGHT = 1.55 # Starts exactly at shoulder height
    HOLE_RADIUS = 0.16 # Clears the head (0.11) but rests on shoulders (0.44 span)
    
    nodes = {} # Store valid coordinates
    ...
    # Create Rigid Bodies (Chainmail Rings)
    for (r, c), (x, y) in nodes.items():
        node_def = f"ClothNode_{r}_{c}"
        rad = SPACING * 0.48 # Nearly touching
        xml.append(f'<RigidBody DEF="{node_def}" position="{x:.4f} {y:.4f} {Z_HEIGHT}" mass="0.05" linearDampingFactor="0.6" angularDampingFactor="0.8">')
        xml.append(f'<CollidableShape containerField="geometry"><Shape containerField="shape">')
        xml.append('<Appearance><Material diffuseColor="0.8 0.1 0.3" shininess="0.5"/></Appearance>')
        xml.append(f'<Sphere radius="{rad:.4f}"/>')
        xml.append('</Shape></CollidableShape></RigidBody>')
        
    # Create BallJoint Links (Springs)
    for (r, c), (x, y) in nodes.items():
        # Link to Right
        if (r+1, c) in nodes:
            nx, ny = nodes[(r+1, c)]
...
def main():
    ...
    # Generate Structural Baseline
    arm_obj = create_armature()
    skin_obj = build_skin(arm_obj)
    
    # Setup poses (No Blender Physics calculations needed!)
    animate_humanoid(arm_obj)
    
    # Export real-time calculated physics standard directly
    home_dir = os.path.expanduser("~")
    export_path = os.path.join(home_dir, "hanim_rigidbody_poncho.x3d")
