...
def build_skin(arm_obj):
    ...
    for bone in arm_obj.data.edit_bones:
        ...
        elif "phalanx" in bone.name or "carpal" in bone.name: r = 0.008 
...
def extract_clothing(skin_obj, name, z_min, z_max, expand):
    ...
    for v in bm.verts: v.co += v.normal * expand
...
def build_poncho():
    # PRE-THREADED POSITION: Z=1.55m is exactly neck height (above clavicles, below head radius).
    # The head inherently starts physically sticking through the pre-cut hole.
    poncho_z_height = 1.55
    bpy.ops.mesh.primitive_grid_add(x_subdivisions=65, y_subdivisions=65, size=1.8, location=(0, 0, poncho_z_height))
    poncho = bpy.context.active_object
    poncho.name = "Poncho"
    
    # Cut circular neck hole exactly around the neck coordinate
    bpy.ops.object.mode_set(mode='EDIT')
    bm = bmesh.from_edit_mesh(poncho.data)
    bm.verts.ensure_lookup_table()
    center = mathutils.Vector((0, 0, poncho_z_height))
    # Hole radius ~0.14m is perfect to slip past the skull bounds (r=0.10) but catch on shoulders
    ...
    cloth.settings.bending_stiffness = 0.1 # Very low resistance allows tight draping
    
    cloth.collision_settings.use_collision = True
    cloth.collision_settings.use_self_collision = True
    cloth.collision_settings.friction = 40.0 # High friction prevents sliding off
...
def bind_and_physics(arm_obj, skin, blouse, pants):
    for obj in [skin, blouse, pants]: obj.select_set(True)
    ...
    for obj in [skin, blouse, pants]:
        ...
        col.settings.thickness_outer = 0.03
        col.settings.cloth_friction = 35.0 
...
def main():
    ...
    export_path = os.path.join(home_dir, "hanim_poncho_simulation2.x3d")
