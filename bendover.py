...
def create_humanoid_system():
    ...
    # Apply Modifiers (Destructive for export weights)
    ...
    # Increased thickness to prevent poncho clipping
...
def create_poncho():
    # Widened size slightly to fit over arms better
...
def animate_touch_toes(arm_obj, start_frame, end_frame):
    ...
    # Define bone chains
    spine_bones = []
    # Collect all L and T and C bones
    ...
    for f in [1, start_frame]:
        ...
        for bname in key_bones:
            ...
            if pb:
                ...
                pb.location = (0,0,0) # Local reset
                pb.rotation_euler = (0,0,0)
                pb.keyframe_insert(data_path="rotation_euler")
                pb.keyframe_insert(data_path="location")

    # 2. End Frame (Natural Bending)
    scene.frame_set(end_frame)

    # A. Spine Curl (Cumulative)
    # 90 degrees total bend distributed across ~24 vertebrae = ~3.75 deg per bone
    ...
    # B. Pelvis Tilt (Flexion)
    # We tilt the sacrum forward
    ...
    # C. Keep Legs Vertical (Counter-rotation)
    # If sacrum tilts +50, thighs must rotate -50 relative to pelvis to stay straight
    ...
    # D. Shift Weight (Keep feet on ground)
    # Moving the root back and down to compensate for the torso shifting forward
    root = arm_obj.pose.bones.get('humanoid_root')
    if root:
        # Move back (Y) and Down (Z)
...
def export_complex_x3d(filepath, arm_obj, skin_obj, poncho_obj, start_frame, end_frame):
    ...
    # 1. Extract Weights (Fix for Attribute Error)
    ...
    def write_joint(f, bone, indent):
        ...
        f.write(f'{indent}<HAnimJoint DEF="{bone.name}" name="{bone.name}" center="{center}" skinCoordIndex="{s_idx}" skinCoordWeight="{s_wgt}">\n')
    ...
    with open(filepath, 'w', encoding='utf-8') as f:
        ...
        for bone in arm_obj.pose.bones:
            # Only export if animated (has keys or is root)
            # Simplification: Export all for HAnim completeness
            rot_values = []
            for fr in range(start_frame, end_frame + 1):
            ...
            # Root Translation needs PositionInterpolator
            if bone.name == "humanoid_root":
                pos_values = []
                for fr in range(start_frame, end_frame + 1):
                    bpy.context.scene.frame_set(fr)
                    # Use bone.location (Local translation)
                    # Swap Y/Z for X3D
                    l = bone.location
