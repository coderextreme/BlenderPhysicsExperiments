...
def build_skin(arm_obj):
    ...
    # Auto Weight Bind ensures Vertex Groups perfectly map to HAnim Joints
...
def animate_humanoid(arm_obj):
    ...
    # Bending towards +X (Forward) requires rotating around Local Z when Bone Roll is 0!
...
def export_x3d_physics(filepath, arm_obj, skin_obj):
    ...
    # --- FIX 2: MAKE THE CLOTH NODES VISIBLE ---
    # We must explicitly create visible Transforms that the physics bodies will control!
    ...
    # Generate the Physics Collection (with Gravity matching Blender's Z-Up!)
    ...
    # --- FIX 3: ROUTE PHYSICS DATA TO RENDER TRANSFORMS ---
...
def main():
    ...
    export_path = os.path.join(home_dir, "hanim_rigidbody_poncho2.x3d")
