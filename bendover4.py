...
def get_loa4_hierarchy():
    ...
    def build_spine():
        ...
        l_arm = ("l_sternoclavicular", (0.04,0,0.02), 0.06, [("l_acromioclavicular", (0.06,0,0), 0.05, [("l_shoulder", (0.05,0,-0.02), 0.05, [("l_upperarm", (0.1,0,-0.05), 0.07, [("l_elbow", (0.28,0,0), 0.06, [("l_forearm", (0.25,0,0), 0.05, [("l_wrist", (0.05,0,0), 0.04, get_hand_digits("l"))])])])])])])
        r_arm = ("r_sternoclavicular", (-0.04,0,0.02), 0.06, [("r_acromioclavicular", (-0.06,0,0), 0.05, [("r_shoulder", (-0.05,0,-0.02), 0.05, [("r_upperarm", (-0.1,0,-0.05), 0.07, [("r_elbow", (-0.28,0,0), 0.06, [("r_forearm", (-0.25,0,0), 0.05, [("r_wrist", (-0.05,0,0), 0.04, get_hand_digits("r"))])])])])])])
    ...
    legs = [("pelvis", (0,0,-0.05), 0.14, [
            ("l_hip", (0.1,0,-0.1), 0.1, [("l_thigh", (0.05,0,-0.1), 0.11, [("l_knee", (0,0,-0.4), 0.09, [("l_calf", (0,0,-0.4), 0.08, [("l_ankle", (0,0,-0.05), 0.06, [("l_calcaneus", (0, -0.05, -0.05), 0.05, [("l_navicular", (0, 0.05, 0), 0.04, get_foot_digits("l"))])])])])])]),
            ("r_hip", (-0.1,0,-0.1), 0.1, [("r_thigh", (-0.05,0,-0.1), 0.11, [("r_knee", (0,0,-0.4), 0.09, [("r_calf", (0,0,-0.4), 0.08, [("r_ankle", (0,0,-0.05), 0.06, [("r_calcaneus", (0, -0.05, -0.05), 0.05, [("r_navicular", (0, 0.05, 0), 0.04, get_foot_digits("r"))])])])])])])])]
...
def create_humanoid_system():
    ...
    # 4. Collision (CRITICAL FOR PONCHO)
    bpy.ops.object.select_all(action='DESELECT')
    skin_obj.select_set(True)
    bpy.context.view_layer.objects.active = skin_obj
    bpy.ops.object.modifier_add(type='COLLISION')
    # Large outer thickness to prevent clipping
    skin_obj.modifiers["Collision"].settings.thickness_outer = 0.06
    skin_obj.modifiers["Collision"].settings.thickness_inner = 0.02
    
    return arm_obj, skin_obj

def create_poncho():
    bpy.ops.mesh.primitive_plane_add(size=2.1, location=(0,0,2.1))
    ...
    to_del = [f for f in bm.faces if abs(f.calc_center_median().x)<0.15 and abs(f.calc_center_median().y)<0.15]
    bmesh.ops.delete(bm, geom=to_del, context='FACES')
    bmesh.update_edit_mesh(poncho.data)
    bpy.ops.object.mode_set(mode='OBJECT')
    
    # Cloth Settings (High Quality)
    bpy.ops.object.modifier_add(type='CLOTH')
    cmod = poncho.modifiers["Cloth"]
    cmod.settings.quality = 12
    cmod.settings.mass = 0.5
    cmod.settings.tension_stiffness = 8
    cmod.settings.bending_model = 'ANGULAR'
    cmod.settings.bending_stiffness = 1.0
    cmod.settings.air_damping = 1.5 # Slow it down slightly
    ...
    if hasattr(cmod, 'collision_settings'):
        ...
        # Use impulse clamp to prevent explosions/pass-throughs
        if hasattr(cmod.collision_settings, 'impulse_clamp'):
            cmod.collision_settings.impulse_clamp = 5.0
            
    # Solidify for visuals (After cloth)
    bpy.ops.object.modifier_add(type='SOLIDIFY')
    poncho.modifiers["Solidify"].thickness = 0.01
...
def animate_touch_toes(arm_obj, start_frame, end_frame):
    ...
    spine_bones = [f"l{i}" for i in range(5,0,-1)] + [f"t{i}" for i in range(12,0,-1)] + [f"c{i}" for i in range(7,0,-1)]
    ...
    for bname in spine_bones:
        pb = arm_obj.pose.bones.get(bname)
        if pb: pb.rotation_euler.x = spine_curl; pb.keyframe_insert("rotation_euler")
    
    sacrum = arm_obj.pose.bones.get('sacrum')
    tilt = math.radians(50)
    if sacrum: sacrum.rotation_euler.x = tilt; sacrum.keyframe_insert("rotation_euler")
        
    for thigh in ['l_thigh', 'r_thigh']:
        pb = arm_obj.pose.bones.get(thigh)
        if pb: pb.rotation_euler.x = -tilt; pb.keyframe_insert("rotation_euler")

    # Gentle Root Shift (Balance)
    root = arm_obj.pose.bones.get('humanoid_root')
    if root:
        # Reduced shift to prevent "falling over" look
    ...
    for arm in ['l_shoulder', 'r_shoulder', 'l_upperarm', 'r_upperarm']:
        ...
        if pb: pb.rotation_euler.x = math.radians(35); pb.keyframe_insert("rotation_euler")

    bpy.ops.object.mode_set(mode='OBJECT')

def bake_physics(start, end):
    print("Baking Physics (Quality=12)...")
    scene = bpy.context.scene
...
def export_complex_x3d(filepath, arm_obj, skin_obj, poncho_obj, start_frame, end_frame):
    ...
    # --- Pre-calculate Poncho ---
    print("Sampling Poncho...")
    poncho_keys = []
    poncho_kvs = []
    
    dg = bpy.context.evaluated_depsgraph_get()
    
    # Capture Frame 1 for <Coordinate point="...">
    ...
    with open(filepath, 'w', encoding='utf-8') as f:
        ...
        def write_joint(bone, indent):
            ...
            container = "skeleton" if bone.name == "humanoid_root" else "children"
        ...
        for bone in arm_obj.pose.bones:
            ...
            if bone.name == "humanoid_root":
                ...
                f.write(f'<PositionInterpolator DEF="Root_PosInt" key="{keys_str}" keyValue="{" ".join(pos_values)}" />\n')
